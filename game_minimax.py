...
class Alpha_Beta_Pruning(Minimax):
    ...
    def run(self, depth = 0):
        ...
        score = self.max_step(depth, self.current_state, 'root', alpha, beta)
        self.traverse_file.close()
        
        self.output_next_state(self.next_state_file_name)
        return self.next_state

    def max_step(self, depth, pre_state, parent, alpha, beta):
        if depth == self.cutting_off_depth or terminate(self.left_step):
            score = self.utility(pre_state)
        ...
        for row in range(BOARD_SIZE):
            for col in range(BOARD_SIZE):
                if pre_state[row][col] == '*':
                    ...
                    score = max(score, self.min_step(depth+1, next_state_try, position, alpha, beta))
    ...
    def min_step(self, depth, pre_state, parent, alpha, beta):
        if depth == self.cutting_off_depth or terminate(self.left_step):
            score = self.utility(pre_state)
            self.traverse_file.write('%s,%s,%s,%s,%s\n' % (parent,depth,self.num2str(score),self.num2str(alpha),self.num2str(beta)))
            return score 
        ...
        for row in range(BOARD_SIZE):
            for col in range(BOARD_SIZE):
                if pre_state[row][col] == '*':
                    ...
                    score = min(score, self.max_step(depth+1, next_state_try, position, alpha, beta))
                    if score <= alpha:
                        self.traverse_file.write('%s,%s,%s,%s,%s\n' % (parent,depth,self.num2str(score),self.num2str(alpha),self.num2str(beta)))
                        return score
                    beta = min(beta, score)
